...
class AppStore extends Component {
  state = {activeTabCategory: tabsList[0].tabId, searchInput: ''}

  onClickTab = tabId => {
    this.setState({activeTabCategory: tabId})
  }
  ...
  getActiveTabApps = searchedApps => {
    const {activeTabCategory} = this.state
    const filteredApps = searchedApps.filter(
      eachApp => eachApp.category === activeTabCategory,
    )
    return filteredApps
  }
  ...
  render() {
    const {activeTabCategory, searchInput} = this.state
    const searchApps = this.getSearchResults()
    const filteredApps = this.getActiveTabApps(searchApps)
    return (
        ...
        <div className="content-card">
          ...
          <ul className="tabs-container">
            {tabsList.map(eachItem => (
              <TabItem
                isActive={eachItem.tabId === activeTabCategory}
                key={eachItem.tabId}
                tabDetails={eachItem}
                tabClicked={this.onClickTab}
              />
            ))}
          </ul>
